...
class DriverInfoComponent(BaseComponent):
    ...
    def draw(self, window):
        ...
        box_height = 140 # Slightly compact

        # Position below Weather
        # We assume 'window' has 'weather_bottom' set by race_replay layout logic
        weather_bottom = getattr(window, "weather_bottom", window.height - 200)
        
        top = weather_bottom - 20
        ...
        # Red Border
        arcade.draw_rect_outline(rect, (200, 0, 0, 255), 2)

        # Header (Red)
        header_height = 30
        header_cy = top - (header_height / 2)
        header_rect = arcade.XYWH(center_x, header_cy, box_width, header_height)
        arcade.draw_rect_filled(header_rect, (200, 0, 0, 255))

        arcade.Text(f"Driver {code}", left + 10, header_cy,
                    arcade.color.WHITE, 14, anchor_y="center", bold=True).draw()

        # Body Text
        cursor_y = top - header_height - 20
        left_text_x = left + 15
        row_gap = 22

        # (A) Speed
        speed = driver_pos.get('speed', 0)
        arcade.Text(f"Speed: {speed:.1f} km/h", left_text_x, cursor_y, arcade.color.WHITE, 12, anchor_y="center").draw()
        cursor_y -= row_gap
        ...
        arcade.Text(drs_str, left_text_x, cursor_y, arcade.color.WHITE, 12, anchor_y="center").draw()
        cursor_y -= row_gap
        
        # (D) Current Lap (from frame)
        current_lap = frame.get('lap', '-')
        arcade.Text(f"Current Lap: {current_lap}", left_text_x, cursor_y, arcade.color.WHITE, 12, anchor_y="center").draw()
